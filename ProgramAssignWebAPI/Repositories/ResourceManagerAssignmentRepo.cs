  ...
  public class ResourceManagerAssignmentRepo : IResourceManagerAssignmentRepo
    {
        ...
        public async Task<ResourceManagerAssignmentsHistory> UpdateResourceHistory(int id, ResourceManagerAssignmentsHistory resourcehistory, string category)
        {
                ...
                if (resourcefromdb != null)
                {
                    ...
                    var getHistroyAssignedPrograms = _dbContext.ResourceManagerAssignmentsHistory.Where(x => x.VAMID == resourcehistory.VAMID).Select(x => x.HistoryProgramTrackerId).ToArray();
                    List<int> historyassignedPrograms = new List<int>();
                    foreach (var item in getHistroyAssignedPrograms)
                    {
                        foreach (string s in item.Split(','))
                        {
                            historyassignedPrograms.Add(int.Parse(s));
                        }

                    }
                    var totalExcludedPrograms = getPresentAssignedProgram.Concat(historyassignedPrograms).ToArray();
                    ...
                    var resultPrograms = getAllPrograms.Except(totalExcludedPrograms).ToArray();
                    //if (resultPrograms.Length == 0)
                    //{
                    //    throw new Exception ("all programs assigned");
                    //}
